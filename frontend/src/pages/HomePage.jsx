...
const categoryBanners = {
    livebirds: {
        title: "Premium Fresh Live Birds & Eggs",
        text: "Our <strong>Natu Kodi birds and eggs</strong> embody a true commitment to wellness, sourced from ethical farms to bring you the best in flavor and nutrition.",
...
const categoryFeatures = {
    livebirds: {
        title: "Unlock the Power of Natu Kodi",
        subtitle: `"Health is the true wealth for our girl child, girls, and women."`,
        description: "Our Natu Kodi eggs & birds are nutritional dynamos, carefully selected for their extraordinary benefits that empower wellness at every life stage.",
...
const LIVEBIRDS_ORDER = [ 'sonali', 'kadaknath', 'turkey', 'guinea fowl', 'duck', 'rabbit', 'goat' ];
...
export default function HomePage({ handleAddToCart }) {
    ...
    useEffect(() => {
        ...
        const fetchProducts = async () => {
            ...
            setError('');
            try {
                const res = await axios.get(`https://srestamart.com/api/products?page=${currentPage}&limit=12&category=${selectedCategory}`);
                
                if (res.data && Array.isArray(res.data.products)) {
                    let fetchedProducts = res.data.products.map(p => ({ ...p, category: p.category.toLowerCase().replace(/\s+/g, '') }));

                    if (selectedCategory === 'livebirds') {
                        fetchedProducts.sort((a, b) => {
                            const aName = a.name.toLowerCase();
                            const bName = b.name.toLowerCase();
                            const aIndex = LIVEBIRDS_ORDER.findIndex(keyword => aName.includes(keyword));
                            const bIndex = LIVEBIRDS_ORDER.findIndex(keyword => bName.includes(keyword));
