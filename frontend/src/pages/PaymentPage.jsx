...
export default function PaymentPage({ user, checkoutDetails, handleClearCart, isFirstOrder }) {
    ...
    const { discount, shippingCost, finalTotal, totalWeight } = useMemo(() => {
        ...
        // 2. Calculate Total Weight (assuming 1kg per item quantity as a placeholder)
        // IMPORTANT: For real-world accuracy, you should add a 'weight' property to your product variants in the database.
        const totalWeight = checkoutDetails?.items.reduce((acc, item) => acc + (1 * item.quantity), 0) || 0;
