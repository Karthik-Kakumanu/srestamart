...
const parseWeightFromLabel = (label) => {
    ...
    // Match numbers (including decimals) followed by 'kg' or 'g'
    ...
    if (match) {
        ...
        return value; // Value is already in kilograms
    }
    
    return 0; // Return 0 if no weight is found in the label
};
...
const calculateShipping = (subtotal, address, totalWeight) => {
    ...
    if (addressString.includes('hyderabad')) {
        return subtotal > 1500 ? 0 : 50 * totalWeight;
    }
    // Rule 3: Telangana (and not Hyderabad)
    if (addressString.includes('telangana')) {
        return 150 * totalWeight;
    }
    // Rule 4: Andhra Pradesh
    if (addressString.includes('andhra pradesh') || addressString.includes('a.p')) {
        return 200 * totalWeight;
    }
    // Rule 5: Rest of India
    return 350 * totalWeight;
};
                ...
                >
                    <div className="bg-white p-6 rounded-2xl shadow-xl sticky top-28">
                        ...
                        <motion.button 
                            ...
                            className="relative w-full flex items-center justify-center mt-6 bg-green-600 text-white font-bold py-4 rounded-lg hover:bg-green-700 transition-all text-lg shadow-lg hover:shadow-xl disabled:bg-gray-400 disabled:cursor-not-allowed overflow-hidden"
                        >
                            {isSubmitting ? (
                                <>
                                    <motion.div
                                        ...
                                        initial={{ x: '-100%' }}
                                        animate={{ x: '450%' }} // Large value to ensure it crosses wide buttons
                                        transition={{
                                            duration: 2.5,
                                            repeat: Infinity,
                                            ease: 'linear',
