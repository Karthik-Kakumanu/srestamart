...
export default function AuthPage({ setLoggedInUser }) {
  ...
  return (
    <div className="min-h-screen flex flex-col md:flex-row bg-slate-100">
      <div 
        className="w-full md:w-1/2 lg:w-3/5 min-h-[50vh] md:min-h-screen flex flex-col justify-end p-8 sm:p-12 text-white relative bg-cover bg-center"
        style={{ backgroundImage: `url(${brandingImageUrl})` }}
      >
        <div className="absolute inset-0 bg-black/60" aria-hidden="true"></div>
         {/* LOGO AND BRAND NAME REMOVED FROM HERE */}
        <motion.div initial={{opacity: 0, y: 20}} animate={{opacity: 1, y: 0}} transition={{duration: 0.6, delay: 0.2}} className="relative">
           {/* INCREASED FONT SIZE HERE */}
          <h1 className="text-5xl lg:text-6xl font-bold tracking-tight leading-tight">Healthy is always wealthy.</h1>
          <p className="mt-4 text-xl text-slate-300">Your one-stop shop for fresh, tangy, and tasty products.</p>
        </motion.div>
      </div>
      <div className="w-full md:w-1/2 lg:w-2/5 flex items-center justify-center p-6 sm:p-12">
        <div className="max-w-md w-full">
          <motion.div
            ...
            className="w-full"
          >
            {/* LOGO MOVED HERE, CENTERED AND SIZED */}
            <div className="text-center mb-6">
                <img src={logoUrl} alt="Sresta Mart Logo" className="h-24 w-24 mx-auto" />
            </div>

            <div className="text-center mb-8">
               {/* INCREASED FONT SIZE AND CENTERED TEXT HERE */}
              <h2 className="text-4xl font-bold text-gray-900 tracking-tight">{getTitle()}</h2>
              <p className="mt-2 text-md text-gray-500">{getSubtitle()}</p>
            </div>
